...
exports.getAllPosts = (data, callback) => {
  db.query(
    `SELECT p.id AS postId, p.description, p.datetimeCreated, 
     p.likeCount, p.dislikeCount, p.addedByUserId,  
     u.firstName, u.lastName FROM posts AS p  INNER JOIN users AS u ON p.addedByUserId = u.id `,
    [],
    (error, results, fields) => {
      if (error) {
        return callback(error);
...
exports.getPostAllComments = (data, callback) => {
  db.query(
    `SELECT c.comment, c.datetimeCreated, c.addedByUserId, u.firstName, u.lastName FROM comments AS c
    INNER JOIN users AS u ON c.addedByUserId = u.id WHERE c.postId = ?`,
    [data.postId],
    (error, results, fields) => {
      if (error) {
        return callback(error);
  ...
  db.query(
    ...
    (error, results, fields) => {
      ...
      if (results.affectedRows === 1) {
        ...
        return callback(null, "Invalid Post");
      }
    }
  );
};
